    ...
    <div className={`relative ${className}`} ref={containerRef}>
      ...
      <div
        ...
        className={`w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-800 border rounded-xl text-xs font-medium flex items-center justify-between cursor-pointer transition-all shadow-2xs select-none ${
          isOpen
            ? 'border-[#3B52D4] ring-2 ring-[#3B52D4]/20 bg-white dark:bg-slate-800'
      ...
      >
        <div className="flex items-center gap-2.5 truncate">
          <CalendarIcon className={`w-4 h-4 shrink-0 ${value ? 'text-[#3B52D4]' : 'text-slate-400'}`} />
        ...
        <div className="absolute z-50 mt-2 p-4 w-72 sm:w-80 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-2xl animate-in fade-in zoom-in-95 duration-150">
            ...
            <div className="flex items-center gap-1.5">
              <button
                ...
                className={`flex items-center gap-1 px-2.5 py-1 rounded-xl text-xs font-extrabold transition cursor-pointer select-none border ${
                  showMonthMenu
                    ? 'bg-[#486B1C] text-white border-[#486B1C] shadow-xs'
                  ...
                  return (
                    <button
                      ...
                      className={`py-2.5 px-2 rounded-xl text-xs font-bold transition-all cursor-pointer text-center ${
                        isCurrent
                          ? 'bg-[#486B1C] text-white shadow-sm font-extrabold'
                          : 'bg-slate-50 dark:bg-slate-800/80 hover:bg-lime-50 dark:hover:bg-lime-950/40 text-slate-700 dark:text-slate-200 hover:text-[#486B1C] dark:hover:text-lime-400 border border-slate-100 dark:border-slate-800'
                      }`}
                    >
                      {name.slice(0, 3)}
                    </button>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {presets.map((preset) => (
                      <button
                        ...
                        className="px-2.5 py-1 rounded-lg bg-slate-100 dark:bg-slate-800 hover:bg-lime-50 dark:hover:bg-lime-950/40 hover:text-[#486B1C] dark:hover:text-lime-400 text-[11px] font-bold text-slate-700 dark:text-slate-300 transition cursor-pointer"
                      >
                        {preset.label}
                      </button>
                    ))}
                    ...
                    <button
                      ...
                      className={`h-8 rounded-xl text-xs font-bold transition-all flex items-center justify-center relative cursor-pointer ${
                        selected
                          ? 'bg-[#486B1C] text-white shadow-sm font-extrabold'
                          : todayFlag
                          ? 'border-2 border-[#486B1C] text-[#486B1C] font-extrabold hover:bg-lime-50 dark:hover:bg-lime-950/30'
          ...
          <div className="mt-3 pt-2.5 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
            <button
              ...
              className="text-xs font-extrabold text-[#486B1C] hover:underline cursor-pointer"
            >
              Go to Today
            </button>
            <button
