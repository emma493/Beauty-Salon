...
export const AdminProducts: React.FC = () => {
  ...
  const handleApplyStockAdjust = (productId: string) => {
    const amount = parseInt(adjustQtyInput, 10);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs font-medium text-slate-400 mt-0.5">Manage inventory, prices, and stock updates</p>
        </div>

        {/* Search, Layout Toggle & Add Product Button */}
        <div className="flex flex-wrap items-center gap-3">
          <div className="relative w-full sm:w-64">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-full text-xs font-medium text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-[#486B1C] shadow-2xs"
            />
          </div>
          ...
          <button
            ...
            className={`p-2.5 rounded-full border transition ${
              showFilters || selectedLocation !== 'all'
                ? 'bg-[#486B1C] text-white border-[#486B1C]'
          ...
          <div className="flex items-center bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-1 rounded-full shadow-2xs">
            <button
              ...
              className={`p-1.5 rounded-full transition ${
                viewMode === 'grid' ? 'bg-[#486B1C] text-white' : 'text-slate-400 hover:text-slate-600'
              }`}
              title="Grid View"
            >
            ...
            <button
              ...
              className={`p-1.5 rounded-full transition ${
                viewMode === 'list' ? 'bg-[#486B1C] text-white' : 'text-slate-400 hover:text-slate-600'
              }`}
              title="List View"
            >
          ...
          <button
            ...
            className="flex items-center gap-2 px-5 py-2.5 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 text-slate-900 dark:text-white font-extrabold text-xs rounded-full hover:bg-slate-50 dark:hover:bg-slate-800 transition active:scale-95 shadow-2xs shrink-0"
          >
            <PlusCircle className="w-4 h-4 text-[#486B1C]" />
      ...
      {filteredProducts.length === 0 ? (
        <div className="bg-white dark:bg-slate-900 rounded-3xl p-12 text-center text-slate-400 border border-slate-200 dark:border-slate-800 space-y-3">
          ...
          <p className="text-xs text-slate-400">Add products using the button above or clear search filters.</p>
        </div>
      ) : viewMode === 'grid' ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
          {filteredProducts.map((p) => {
            ...
            return (
              <div
                key={p.id}
                className="bg-white dark:bg-slate-900 rounded-3xl p-3 border border-slate-200/80 dark:border-slate-800 hover:shadow-lg transition-all duration-300 flex flex-col justify-between group space-y-3 relative"
              >
                {/* Image Card Container */}
                <div
                  onClick={() => setDetailProduct(p)}
                  className="relative w-full aspect-square bg-[#F4F5F7] dark:bg-slate-800/80 rounded-2xl p-4 flex items-center justify-center overflow-hidden cursor-pointer"
                >
                  {p.imageUrl ? (
                    <img
                      src={p.imageUrl}
                ...
                {/* Info Section */}
                <div className="px-1 space-y-1 cursor-pointer" onClick={() => setDetailProduct(p)}>
                ...
                {/* Bottom Stock Stats & Stock Adjust Button */}
                <div className="px-1 pt-1 border-t border-slate-100 dark:border-slate-800 space-y-2">
                  ...
                  {isAdjusting ? (
                    <div className="flex items-center gap-1.5 p-1 bg-slate-100 dark:bg-slate-800 rounded-xl">
                      <button
                        onClick={() => setAdjustMode(adjustMode === 'add' ? 'remove' : 'add')}
                        className={`p-1 rounded-lg font-bold text-[10px] ${
                          adjustMode === 'add' ? 'bg-emerald-600 text-white' : 'bg-rose-600 text-white'
                        }`}
                      ...
                      <input
                        ...
                        placeholder="Qty"
                        className="w-12 px-1.5 py-1 bg-white dark:bg-slate-900 text-xs font-bold rounded-lg border border-slate-200 dark:border-slate-700 text-center"
                      />
                      <button
                        onClick={() => handleApplyStockAdjust(p.id)}
                        className="p-1 bg-[#486B1C] text-white rounded-lg hover:bg-emerald-700 transition"
                      >
            ...
            return (
              ...
              >
                <div className="flex items-center gap-3.5 flex-1 min-w-0">
                  <div className="w-12 h-12 rounded-2xl bg-[#F4F5F7] dark:bg-slate-800 overflow-hidden shrink-0 flex items-center justify-center p-1">
                    {p.imageUrl ? (
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-slate-400">Cost: {currencySymbol}{p.costPrice.toFixed(2)} | Category: {p.categoryName || 'General'} | Sold: {soldCount}</p>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between sm:justify-end gap-3 shrink-0">
                  <div className="text-right">
                    ...
                    <div className="text-base font-black text-[#486B1C] dark:text-emerald-400">
                      {currencySymbol}{p.sellingPrice.toFixed(2)}
                    </div>
                  </div>
                  ...
                  <button
                    onClick={() => setDetailProduct(p)}
                    className="flex items-center gap-1.5 px-3 py-2 bg-slate-100 hover:bg-[#486B1C] hover:text-white dark:bg-slate-800 text-slate-700 dark:text-slate-200 font-extrabold text-xs rounded-xl transition"
                  >
                  ...
                  {isAdjusting ? (
                    ...
                    <button
                      ...
                      className="px-3 py-2 bg-[#486B1C] text-white text-xs font-extrabold rounded-xl hover:bg-emerald-700 transition flex items-center gap-1"
                    >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-xs p-4">
          <div className="relative w-full max-w-lg bg-white dark:bg-slate-900 rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-800 p-6 space-y-5">
            <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-4">
              <div className="flex items-center gap-3">
                <div className="w-12 h-12 rounded-2xl bg-[#F4F5F7] dark:bg-slate-800 p-1 flex items-center justify-center shrink-0 border border-slate-200 dark:border-slate-700">
            ...
            <div className="grid grid-cols-2 gap-3 text-xs">
              <div className="p-3 bg-slate-50 dark:bg-slate-800/50 rounded-2xl space-y-0.5">
                <span className="text-slate-400 font-semibold">Cost Price</span>
                <div className="font-extrabold text-slate-900 dark:text-white">
                  {currencySymbol}{detailProduct.costPrice.toFixed(2)}
                </div>
              </div>

              <div className="p-3 bg-slate-50 dark:bg-slate-800/50 rounded-2xl space-y-0.5">
                <span className="text-slate-400 font-semibold">Selling Price</span>
                <div className="font-extrabold text-base text-[#486B1C] dark:text-emerald-400">
            ...
            <div className="p-3 bg-slate-50 dark:bg-slate-800/50 rounded-2xl space-y-1 text-xs">
              <span className="text-slate-400 font-semibold">Supplier & Description</span>
              <p className="text-slate-700 dark:text-slate-300 font-medium leading-relaxed">
                {detailProduct.supplierInfo ? `Supplier: ${detailProduct.supplierInfo}. ` : ''}
