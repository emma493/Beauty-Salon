...
export const AdminFeedback: React.FC = () => {
  ...
  return (
    <div className="space-y-4 max-w-6xl mx-auto font-sans">
      {/* Top Header */}
      <div className="bg-[#00a884] text-white p-4 rounded-t-3xl shadow-md flex items-center justify-between">
      ...
      <div className="bg-[#efeae2] dark:bg-[#0b141a] rounded-b-3xl border border-slate-300 dark:border-slate-800 shadow-xl overflow-hidden grid grid-cols-1 md:grid-cols-3 h-[580px]">
        {/* Left Column: Staff Contacts List */}
        <div className="border-r border-slate-300 dark:border-slate-800 flex flex-col bg-white dark:bg-[#111b21]">
          {/* Search Contacts Bar */}
          <div className="p-3 bg-[#f0f2f5] dark:bg-[#202c33] border-b border-slate-200 dark:border-slate-800">
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-9 pr-3 py-1.5 bg-white dark:bg-[#111b21] border border-slate-200 dark:border-slate-700 rounded-full text-xs font-medium text-slate-900 dark:text-white"
              />
            </div>
          </div>
                ...
                return (
                  <button
                    ...
                    className={`w-full p-3 flex items-center gap-3 text-left transition ${
                      isSelected
                        ? 'bg-[#e9edef] dark:bg-[#2a3942] border-l-4 border-[#00a884]'
        ...
        <div className="col-span-2 flex flex-col h-full bg-[#efeae2] dark:bg-[#0b141a]">
          {activeWorker ? (
            <>
              {/* Active Chat Header */}
              <div className="p-3 px-4 bg-[#f0f2f5] dark:bg-[#202c33] border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-full overflow-hidden border-2 border-[#00a884] shrink-0">
                ...
                <div className="relative w-40 sm:w-48">
                  ...
                  <input
                    ...
                    className="w-full pl-8 pr-2 py-1 bg-white dark:bg-[#111b21] border border-slate-200 dark:border-slate-700 rounded-full text-[11px] outline-none text-slate-900 dark:text-white"
                  />
                </div>
              </div>
                      ...
                      >
                        <div
                          className={`max-w-[85%] sm:max-w-md px-3.5 py-2 rounded-2xl text-xs shadow-xs relative transition-all ${
                            isAdminMsg
                              ? 'bg-[#d9fdd3] dark:bg-[#005c4b] text-slate-900 dark:text-white rounded-tr-none border border-emerald-200/50 dark:border-emerald-800/40'
                        ...
                        >
                            ...
                            <div className="mt-2 pt-2 border-t border-slate-200 dark:border-slate-700">
                              {m.attachmentType === 'image' ? (
                                ...
                                <a
                                  ...
                                  target="_blank"
                                  rel="noreferrer"
                                  className="flex items-center gap-2 p-2 bg-black/5 dark:bg-white/10 rounded-xl hover:bg-black/10 transition"
                                >
                          ...
                          >
                            <span>{timeStr}</span>
                            {isAdminMsg && <CheckCheck className="w-3.5 h-3.5 text-[#53bdeb] inline" />}
                          </div>
                        </div>
                      </div>
                    );
              ...
              {/* Quick Emojis */}
              {showEmojiPicker && (
                <div className="px-4 py-2 bg-slate-100 dark:bg-[#111b21] border-t border-slate-200 dark:border-slate-800 flex items-center gap-2 overflow-x-auto text-base">
                  {['😊', '👍', '❤️', '🙏', '🛍️', '📦', '✅', '⚡', '💯', '👋'].map((e) => (
                    <button
                      key={e}
                      type="button"
              ...
              {showAttachInput && (
                <div className="p-3 bg-slate-100 dark:bg-[#111b21] border-t border-slate-200 dark:border-slate-800 flex items-center gap-2 text-xs">
                  ...
                  <input
                    ...
                    className="flex-1 px-3 py-1.5 bg-white dark:bg-[#202c33] border border-slate-300 dark:border-slate-700 rounded-xl"
                  />
                  <button
                    type="button"
                    onClick={() => setAttachmentType('image')}
                    className={`px-3 py-1 rounded-xl text-[10px] font-bold ${
                      attachmentType === 'image' ? 'bg-[#00a884] text-white' : 'bg-slate-200 text-slate-700'
                    }`}
                  >
                    Image
                  </button>
                  <button
                    ...
                    className={`px-3 py-1 rounded-xl text-[10px] font-bold ${
                      attachmentType === 'document' ? 'bg-[#00a884] text-white' : 'bg-slate-200 text-slate-700'
                    }`}
                  >
                    Document
                  </button>
              ...
              {isRecording ? (
                <div className="p-3 bg-[#f0f2f5] dark:bg-[#202c33] border-t border-slate-200 dark:border-slate-800 flex items-center justify-between gap-3 animate-pulse">
                  <div className="flex items-center gap-3">
                    <button
                      type="button"
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="flex items-center gap-1.5 px-4 py-2 bg-[#00a884] hover:bg-[#008f6f] text-white font-bold text-xs rounded-full shadow-md transition"
                    >
                ...
                <form
                  ...
                  className="p-3 bg-[#f0f2f5] dark:bg-[#202c33] border-t border-slate-200 dark:border-slate-800 flex items-center gap-2"
                >
                  <button
                    type="button"
                    onClick={() => setShowEmojiPicker((prev) => !prev)}
                    className="p-2 text-slate-500 dark:text-slate-400 hover:text-[#00a884] rounded-full transition"
                  >
                  ...
                  <input
                    ...
                    className="flex-1 px-4 py-2.5 bg-white dark:bg-[#2a3942] border border-slate-200 dark:border-slate-700 rounded-2xl text-xs font-medium text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-[#00a884]"
                  />

                  <button
                    type="button"
                    onClick={startVoiceRecording}
                    className="p-2.5 text-slate-500 dark:text-slate-400 hover:text-[#00a884] hover:bg-slate-200 dark:hover:bg-slate-800 rounded-full transition"
                  ...
                  <button
                    type="submit"
                    className="p-2.5 bg-[#00a884] hover:bg-[#008f6f] text-white rounded-full shadow-md flex items-center justify-center transition"
                  >
                    <Send className="w-4 h-4" />
                  </button>
                </form>
