...
export const AdminLogs: React.FC = () => {
  ...
  // Filter storewide logs by date
  const filteredLogs = logs.filter((l) => l.date === selectedDate || l.timestamp.startsWith(selectedDate));
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-bold text-slate-900 dark:text-white">Storewide System Activity Logs</h2>
            <p className="text-xs text-slate-500">Monitor all logins, logouts, transactions, and system updates</p>
          </div>
        </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-xs overflow-hidden">
        <div className="p-4 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
          <h3 className="text-base font-bold text-slate-900 dark:text-white flex items-center gap-2">
            ...
            <span>Store Activity Records for {selectedDate}</span>
          </h3>
        ...
        {filteredLogs.length === 0 ? (
          <div className="p-12 text-center text-slate-400 space-y-2">
            ...
            <p className="text-sm font-medium">No system activity logged for this date ({selectedDate}).</p>
          </div>
        ) : (
