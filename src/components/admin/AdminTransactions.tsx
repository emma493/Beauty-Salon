  ...
  return (
    <div className="space-y-6 pb-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 font-medium mt-0.5">
            Storewide transaction history and POS receipt archives
          </p>
        </div>
        ...
        <div className="flex items-center gap-2">
          <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-emerald-50 dark:bg-emerald-950/50 text-emerald-700 dark:text-emerald-300 text-xs font-bold border border-emerald-200/60 dark:border-emerald-800">
            ...
            <span>Storewide Audit Active</span>
          </span>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-xs overflow-hidden">
        ...
        <div className="p-4 sm:p-5 border-b border-slate-200/80 dark:border-slate-800 flex flex-col xl:flex-row items-stretch xl:items-center justify-between gap-4">
          <div className="flex items-center gap-2">
          ...
          <div className="flex flex-col sm:flex-row flex-wrap items-stretch sm:items-center gap-2 sm:gap-3">
            ...
            <div className="relative w-full sm:w-56">
              ...
              <input
                ...
                className="w-full pl-9 pr-3 py-2 bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 rounded-xl text-xs font-medium text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-[#486B1C]"
              />
              {searchTerm && (
                <button
            ...
            <div className="w-full sm:w-44">
              <CustomSelect
                ...
                options={[
                  ...
                  ...workers.map((w) => ({ value: w.id, label: `${w.firstName} ${w.lastName}` })),
                ]}
                icon={<UserCheck className="w-3.5 h-3.5 text-slate-400 shrink-0" />}
              />
            </div>
          ...
          <div className="p-16 text-center text-slate-400 space-y-3">
            ...
            {(searchTerm || selectedDate || timePreset !== '24h' || selectedWorkerId !== 'all') && (
              <button
                ...
                className="px-4 py-2 rounded-xl bg-[#486B1C]/10 text-[#486B1C] text-xs font-extrabold hover:bg-[#486B1C] hover:text-white transition"
              >
                Clear all filters
              </button>
            )}
          ...
          <>
                  ...
                  {paginatedOrders.map((order) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {/* Worker */}
                        <td className="py-4 px-4 whitespace-nowrap font-semibold text-slate-800 dark:text-slate-200">
                          {order.workerName || 'Staff'}
                        </td>
                        ...
                        <td className="py-4 px-6 text-right whitespace-nowrap">
                          <div className="flex items-center justify-end gap-1.5">
                            <button
                              ...
                              className="px-2.5 py-1.5 rounded-xl bg-slate-100 hover:bg-[#3B52D4] hover:text-white dark:bg-slate-800 dark:hover:bg-[#3B52D4] text-slate-700 dark:text-slate-200 font-bold text-xs transition flex items-center gap-1 cursor-pointer"
                ...
                return (
                  <div key={order.id} className="p-4 space-y-3">
                    ...
                    <div className="flex items-center gap-2 pt-2 border-t border-slate-100 dark:border-slate-800">
                      <button
                        onClick={() => setPreviewOrder(order)}
                        className="flex-1 py-2 bg-slate-100 hover:bg-[#3B52D4] hover:text-white dark:bg-slate-800 dark:hover:bg-[#3B52D4] text-slate-700 dark:text-slate-200 font-bold text-xs rounded-xl transition flex items-center justify-center gap-1.5 cursor-pointer"
                      >
                        <Eye className="w-3.5 h-3.5" />
            ...
            <div className="p-4 border-t border-slate-200/80 dark:border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs font-semibold text-slate-500">
              <div>
                ...
                of <strong className="text-slate-900 dark:text-white">{filteredOrders.length}</strong> entries
              </div>
              ...
              <div className="flex items-center gap-1.5">
                ...
                <div className="px-3 py-1.5 rounded-lg bg-[#3B52D4] text-white font-bold">
                  {currentPage}
                </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-xs p-4">
          <div className="relative w-full max-w-md bg-white dark:bg-slate-900 rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-800 p-6 space-y-5">
            <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-4">
              <div>
                <h3 className="text-lg font-bold text-slate-900 dark:text-white">Admin Receipt Preview</h3>
            ...
            <div className="p-4 bg-white dark:bg-slate-800/90 rounded-2xl border border-emerald-500/30 dark:border-slate-700 space-y-3 text-xs shadow-inner">
              <div className="flex items-center justify-between pb-2 border-b border-slate-200 dark:border-slate-700">
                <div className="flex items-center gap-2">
                  {settings.companyLogoUrl ? (
                    ...
                    <div className="w-6 h-6 rounded-md bg-[#3B52D4] text-white font-black text-xs flex items-center justify-center">
                      POS
                    </div>
                  )}
              ...
              <div className="space-y-0.5 text-slate-600 dark:text-slate-300">
                ...
                <div>
                  Staff Member: <strong className="text-[#3B52D4]">{previewOrder.workerName}</strong>
                </div>
              </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                className="flex-1 py-2.5 bg-[#486B1C] hover:bg-[#3b5817] text-white font-bold text-xs rounded-xl shadow-md flex items-center justify-center gap-1.5 transition cursor-pointer"
              >
