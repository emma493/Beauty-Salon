...
export const UserDashboard: React.FC = () => {
  ...
  // Filter orders by current worker
  const myOrders = orders.filter((o) =>
    currentUser ? o.workerId === currentUser.id : true
  );
  ...
  const topProductsList = sortedProducts.slice(0, 4).map((p, idx) => {
    const bgColors = ['bg-sky-100/80 dark:bg-sky-950/40', 'bg-amber-100/80 dark:bg-amber-950/40', 'bg-purple-100/80 dark:bg-purple-950/40', 'bg-emerald-100/80 dark:bg-emerald-950/40'];
  ...
  return (
    <div className="space-y-6 w-full max-w-full overflow-hidden">
      ...
      {/* ROW 2: SPLIT SECTION - TOP SELLING PRODUCTS & LOW STOCK ITEMS */}
